...
# TODO: create a function make_runner(users)
# The function must:
# - define an inner function runner(action)
# - runner(action) must:
#     - if action is register_user:
#         - call the function
#         - if returned user is not None
#             - append user to users list
#     - otherwise:
#         - call the function with users as argument
# - return the inner function runner
def make_runner(users):
    pass

users = []
running = True
# TODO: create the runner function by calling make_runner(users)
